...
export function ProdutoForm({ id, onCreated }: Props) {
  const isNew = id === "novo";
  ...
  const save = async () => {
    ...
    try {
      ...
      if (isNew) {
        ...
        toast.success("Produto criado com sucesso! Agora adicione as imagens.");
        onCreated(data.id);
      } else {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      ...
      await supabase.from("product_images").insert({
        ...
        url: `products/${path}`,
        ordem: images.length + uploaded,
        principal: images.length === 0 && uploaded === 0,
      });
      uploaded++;
    }
    if (uploaded > 0) toast.success(`${uploaded} imagem(ns) enviada(s)`);
    refetchImages();
    e.target.value = "";
  };
